...
import json
sys.path.append('rp')
from kbc.src.models import SAQE

def load_kbc(model_path, device, nentity, nrelation):
    model = SAQE(sizes=[nentity, nrelation, nentity], rank=1000, init_size=1e-3)
...
def dense_plus(R_dense: torch.Tensor, k: int = 30, base_alpha: float = 0.02): ## 0.02  0.01 0.04,0.06   237:0.4  , nell: 0.04
    sparse_dtype = torch.float16 if R_dense.is_cuda else torch.float32
    device = R_dense.device

    sparsity = (R_dense > 0.001).float().mean()
    if sparsity >0.0005:  # 
    ...
    R_tmp = R_dense.to(sparse_dtype)
    vals, idx = R_tmp.topk(k, dim=1, sorted=False)

    rows = torch.arange(R_tmp.size(0), device=device)\
                 .unsqueeze(1).repeat(1, k).reshape(-1)
    cols = idx.reshape(-1)
    data = vals.reshape(-1)

    R_topk = torch.sparse_coo_tensor(
        torch.stack([rows, cols]), data,
        size=R_tmp.shape, dtype=sparse_dtype, device=device
    ).to_sparse_csr()

    R2_sparse = torch.sparse.mm(R_topk, R_topk)
    R2_dense = R2_sparse.to_dense().to(R_dense.dtype)
    
    if R2_dense.max() > 0:
        r_scale = R_dense.max()
        r2_scale = R2_dense.max()
        if r2_scale > r_scale:
            R2_dense = R2_dense * (r_scale / r2_scale) * 0.4  
    
    return R_dense + alpha * R2_dense
...
def neural_adj_matrix(model, rel, nentity, device, thrshd, adj_list, num_scale=1.0):
    ...
    for s in range(0, nentity, bsz):
        ...
        out=dense_plus(relation_embedding)
    return out
...
class KGReasoning(nn.Module):
    def __init__(self, args, device, adj_list, query_name_dict, name_answer_dict):
        ...
        if args.data_path.split('/')[-1].split('-')[1] == "237":
            dataset_name += "-237"
        filename = '/mnt/shared-storage-user/caozongsheng/neural_adj/'+dataset_name+'_'+str(args.fraction)+'_'+str(args.thrshd)+'_'+str(args.num_scale)+'.pt'
