...
from torch.nn.functional import normalize, softmax     # ← 这里加上 softmax
...
class SAQE(KBCModel):
    ...
    def score(self, x):
        ...
        #print('-----score',lhs[0].shape,rel[0].shape,rhs[0].shape)
    ...
    def row_mean_fast(self,M: torch.Tensor,
                      thresh: float = 0.98,
                      ) -> torch.Tensor:
        #M = M.to(device, non_blocking=True)
        M_norm = normalize(M, dim=1)          # 行向量 L2 归一化    0.95 fb15k, 0.99 237, 
        S = M_norm @ M_norm.T                 # (m, m) 余弦相似度
        mask = (S >= thresh).float()                  # bool
        #alpha = softmax(mask, dim=1)
        #return alpha @ M_norm
        counts = mask.sum(dim=1, keepdim=True).clamp(min=1)
        return (mask.float() @ M) / counts    # (m, n)


    def row_mean_fast1(self,M: torch.Tensor,
                      thresh: float = 0.99,
                      ) -> torch.Tensor:    ###nell 分别5看0.95， 237： 0.99
        #M = M.to(device, non_blocking=True)
        M_norm = normalize(M, dim=1)          # 行向量 L2 归一化
        S = M_norm @ M_norm.T                 # (m, m) 余弦相似度
        mask = (S >= thresh).float()                  # bool
        alpha = softmax(mask, dim=1)
        return alpha @ M
        #counts = mask.sum(dim=1, keepdim=True).clamp(min=1)
        #return (mask.float() @ M_norm) / counts    # (m, n)
    ...
    def forward(self, x, score_rhs=True, score_rel=False, score_lhs=False):
        ...
        lhs = self.row_mean_fast(lhs[:, :self.rank]), self.row_mean_fast(lhs[:, self.rank:])
        rel = self.row_mean_fast(rel[:, :self.rank]), self.row_mean_fast(rel[:, self.rank:])
        rhs =self.row_mean_fast(rhs[:, :self.rank]), self.row_mean_fast(rhs[:, self.rank:])
        
        
        #print('-----forward',lhs[0].shape,rel[0].shape,rhs[0].shape)
        ...
        #print('-----forward',lhs.shape,rel.shape,rhs.shape)
