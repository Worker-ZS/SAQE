...
class NeuralBellmanFordNetwork(nn.Module, core.Configurable):
    ...
    def __init__(self, input_dim, hidden_dims, num_relation, message_func="distmult", aggregate_func="pna",
        ...
        for i in range(len(self.dims) - 1):
            self.layers1.append(layer.CompositionalGraphConv(self.dims[i], self.dims[i + 1], num_relation,
    ...
    def forward(self, graph, input, all_loss=None, metric=None):
        ...
        for layer in self.layers1:
            hidden1 = layer(graph, layer_input)
            if self.short_cut and hidden1.shape == layer_input.shape:
                hidden1 = hidden1 + layer_input
            hiddens1.append(hidden1)
            layer_input = hidden1

        node_query1 = graph.query.expand(graph.num_node, -1, -1)
        if self.concat_hidden:
            node_feature1 = torch.cat(hiddens1 + [node_query], dim=-1)
        else:
            node_feature1 = torch.cat([hiddens1[-1], node_query], dim=-1)

        return {
            "node_feature": node_feature,"node_feature1": node_feature1,
        }
